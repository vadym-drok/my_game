...
export default function Buildings() {
  ...
  return <main><Toast message={message} setMessage={setMessage} /><header className="page-header"><div><p className="eyebrow">{t("Common.nationSimulator")}</p><h1>{t("Buildings.title")}</h1></div>{nation && <p className="page-day">{t("Common.day", {day: nation.current_day})}</p>}</header><section className="grid"><section className="card"><h2>{t("Buildings.available")}</h2>{definitions.map((building) => <div className="building" key={building.code}><strong><ItemIcon item={building} /></strong><span>{dataT(`buildingTypes.${building.building_type}`)} · {building.capacity}</span><ConstructionCost cost={building.construction_cost} resources={resources} /><div className="building-buttons"><button className="button-primary" onClick={() => build(building)}><Hammer aria-hidden="true" />{t("Buildings.build")}</button><button className="button-primary" onClick={() => add(building.code)}><Plus aria-hidden="true" />{t("Common.add")}</button></div></div>)}</section><section className="card"><h2>{t("Buildings.built")}</h2>{built.length === 0 ? <p>{t("Buildings.noneBuilt")}</p> : <div className="built-building-types">{Object.entries(builtByType).map(([type, buildings]) => <section className="building-type" key={type}><h3>{dataT(`buildingTypes.${type}`)}</h3><div className="building-icon-grid">{buildings.map((building) => <button className="button-secondary button-icon building-icon-button" type="button" key={building.id} onClick={() => setSelectedBuilding(building)} aria-label={dataT(`buildings.${building.code}`)}><ItemIcon item={building} /></button>)}</div><p>{t("Buildings.groupSummary", {type: dataT(`buildingTypes.${type}`), capacity: buildings.reduce((total, building) => total + building.capacity, 0)})}</p></section>)}</div>}</section></section>{buildingToConstruct && <div className="modal-backdrop"><form className="modal" onSubmit={startConstruction}><h2>{t("Buildings.construction")}</h2><p>{t("Buildings.availableWorkers", {amount: availableWorkers})}</p><ConstructionCost cost={buildingToConstruct.construction_cost} resources={resources} /><label>{t("Buildings.workers")}<input type="number" min="1" max={availableWorkers} value={constructionWorkers} onChange={(event) => setConstructionWorkers(event.target.value)} required /></label><div><button className="button-secondary" type="button" onClick={() => setBuildingToConstruct(null)}>{t("Common.cancel")}</button><button className="button-primary"><Hammer aria-hidden="true" />{t("Buildings.startConstruction")}</button></div></form></div>}{selectedBuilding && <div className="modal-backdrop"><section className="modal building-details-modal"><button className="button-secondary button-icon modal-close" type="button" onClick={() => setSelectedBuilding(null)} aria-label={t("Common.close")}><X aria-hidden="true" /></button><ItemIcon item={selectedBuilding} type="building_detail" /><h2>{dataT(`buildings.${selectedBuilding.code}`)}</h2><p>{t("Buildings.groupSummary", {type: dataT(`buildingTypes.${selectedBuilding.building_type}`), capacity: selectedBuilding.capacity})}</p><button className="button-danger remove-button" type="button" onClick={() => remove(selectedBuilding.id)}><Trash2 aria-hidden="true" />{t("Common.remove")}</button></section></div>}</main>;
}
