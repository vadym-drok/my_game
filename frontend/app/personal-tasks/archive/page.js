...
export default function PersonalTaskArchivePage() {
  ...
  return <main><Toast message={message} setMessage={setMessage} /><header className="page-header"><div><p className="eyebrow">{t("Common.nationSimulator")}</p><h1>{t("PersonalTasks.archiveTitle")}</h1></div><Link className="page-link" href="/personal-tasks">{t("PersonalTasks.back")}</Link></header><section className="grid"><section className="card"><h2>{t("PersonalTasks.finished")}</h2><TaskList tasks={finished} empty="PersonalTasks.noFinished" t={t} update={update} /></section><section className="card"><h2>{t("PersonalTasks.archived")}</h2><TaskList tasks={archived} empty="PersonalTasks.noArchived" t={t} update={update} /></section></section></main>;
}
