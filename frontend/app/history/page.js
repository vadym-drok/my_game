  ...
  return <main>
    ...
    <section className="card">
      {processes === null ? <p>{t("Common.loading")}</p> : processes.length === 0 ? <p>{t("History.empty")}</p> : <ul className="processes history">
        {processes.map((process) => <li key={process.id}><strong>{dataT(`workTypes.${process.work_type}`, { default: process.work_type })}{process.outputs?.building?.name && `: ${process.outputs.building.name}`}</strong>{process.description && <span>{process.description}</span>}<span className="process-work"><ItemIcon item={workTypesByCode[process.work_type] || { code: process.work_type, name: process.work_type }} type="work_type" />{t(`Modes.${process.mode}`)} · {t(`Statuses.${process.status}`)}</span><span>{t("History.workers", {amount: process.assigned_workers})}{process.mode === "finite" && ` · ${t("History.workerDays", {completed: process.completed_worker_days, required: process.required_worker_days})}`}</span></li>)}
      </ul>}
    </section>
  </main>;
}
